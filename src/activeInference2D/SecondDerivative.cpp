...
double internal_state_mu1p(vector<double> mu, vector<double> a)
{
	if (publish){
		...
		output_mu1p.clear();
		output_mu1p.addDouble(CTRL_RAD2DEG*mu[3]);
		plot_mu1p.write();
	}	
...
double internal_state_mu2p(vector<double> mu, vector<double> a)
{
	if (publish){
		...
		output_mu2p.clear();
		output_mu2p.addDouble(CTRL_RAD2DEG*mu[4]);
		plot_mu2p.write();
	}
...
double internal_state_mu3p(vector<double> mu, vector<double> a)
{
	if (publish){
		...
		output_mu3p.clear();
		output_mu3p.addDouble(CTRL_RAD2DEG*mu[5]);
		plot_mu3p.write();
	}
...
double internal_state_mu1ep(vector<double> mu, vector<double> a)
{
	if (publish){
		...
		output_mu1ep.clear();
		output_mu1ep.addDouble(CTRL_RAD2DEG*mu[2]);
		plot_mu1ep.write();
	}	
...
double internal_state_mu2ep(vector<double> mu, vector<double> a)
{
	if (publish){
		...
		output_mu2ep.clear();
		output_mu2ep.addDouble(CTRL_RAD2DEG*mu[3]);
		plot_mu2ep.write();
	}
...
double internal_state_mu1bp(vector<double> mu, vector<double> a)
{
    if (publish){
        ...
        output_mu1bp.clear();
        output_mu1bp.addDouble(CTRL_RAD2DEG*mu[3]);
        plot_mu1bp.write();
    }
...
double internal_state_mu2bp(vector<double> mu, vector<double> a)
{
    if (publish){
        ...
        output_mu2bp.clear();
        output_mu2bp.addDouble(CTRL_RAD2DEG*mu[4]);
        plot_mu2bp.write();
    }
...
double internal_state_mu3bp(vector<double> mu, vector<double> a)
{
    if (publish){
        ...
        output_mu3bp.clear();
        output_mu3bp.addDouble(CTRL_RAD2DEG*mu[5]);
        plot_mu3bp.write();
    }
