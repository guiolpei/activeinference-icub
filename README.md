...
YARP installation instructions at: https://www.yarp.it/git-master/install_yarp_linux.html

iCub installation instructions at: https://icub-tech-iit.github.io/documentation/sw_installation/linux_binaries/

Eigen and OpenCV are also necessary.
...
We will focus on 3D version, 2D version is deprecated. The necessary *CMakeLists.txt* file is included only for 3D version.

Go to *src/activeInference3D* and run CMake: `cmake .`

Once the necessary files are generated, run `make` to build the executable.

The compiled program is named `./activeInference3D`.

Any problems you might encounter, please feel free to open an issue.
...
2D implementation works with monocular vision with 3 DOF for each one of the arms (shoulder_roll, shoulder_yaw and elbow) and 2 DOF for head movement (neck_pitch and neck_yaw). This version is currently deprecated, only included for historical reasons.
...
Once all the config files are correctly set, the 3D version of the algorithm can be runned using:
...
Note: CLI expects user input after each step of the setup process, being necessary to press the *Enter* key after output of each of the steps is shown.
